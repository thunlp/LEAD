...
client = OpenAI(
    api_key='sk-1lDEcRT2BhZmtblU482a259012Ca4f6a958f7f1359D11fCa',
    base_url="https://yeysai.com/v1/",
    # base_url="https://sailaoda.cn/v1/",
)
parser = argparse.ArgumentParser(description="Help info.")
parser.add_argument('--old', type=str, default='/home/gaocheng/CAIL2022/stage2/query_stage2_valid_onlystage2_40.json', help='Query path.')
parser.add_argument('--w', type=str, default='/home/gaocheng/CAIL2022/stage2/query_stage2_valid_onlystage2_40_simplified.json', help='Write path.')
...
with open(args.w, 'r') as f:
    query_simplify = json.load(f)

query_simplify = []
for line in tqdm(query_old):
    ...
    find = False
    for i in range(len(query_simplify)):
        if query_simplify[i]["ridx"] == ridx:
            if "q_short" in query_simplify[i]:
                find = True
                print(f"{ridx} is ok")
                break
    if find:
    ...
    q_short = response.choices[0].message.content
    print(q_short)

...
class BiEncoderTrainer(object):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        #     print(f"参数 {name} 的数据类型是: {param_type}")
    ...
    def run_train(self):
        ...
        if max_iterations == 0:
            ...
            return
        print("max_iterations", max_iterations, train_iterator.max_iterations)
        ...
        if cfg.train.eval_per_epoch == 1:
            eval_step = max_iterations
    ...
    def validate_nll(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            #             shuffle=False,
            #         )
            #         print(f"error occured in {biencoder_input.question_ids}")
            #     except:
            #         print("error in the second try...")
            #     traceback.print_exc()
            #     continue
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            #             shuffle=False,
            #         )
            #         print(f"error occured in {biencoder_input.question_ids}, in iteration {i}")
            #     except:
            #         print(f"error in the second try... in iteration {i}")
            #     traceback.print_exc()
            #     continue
    ...
    def _save_checkpoint(self, scheduler, epoch: int, offset: int) -> str:
        ...
        if epoch >= cfg.train.num_train_epochs - 1 or epoch == 38 or epoch == 39 or epoch == 40: # 可以修改保存的轮次
    ...
    def _load_saved_state(self, saved_state: CheckpointState):
        ...
        # If you wish to continue pre-training from the DPR checkpoint, set the offset here to 0.
        offset = 0
...
def _calc_loss(
    ...
    if distributed_world_size > 1:
        ...
        # AJAY indicates the charges of the case
        # This is used for the false negatives masking strategy.
        if local_AJAY is not None:
        ...
        for i, item in enumerate(global_question_ctx_vectors):
            if local_AJAY is not None:
                q_vector, ctx_vectors, positive_idx, hard_negatives_idxs, AJAY = item
            else:
            ...
            if i != cfg.local_rank:
                ...
                if local_AJAY is not None:
                    AJAY_per_question.extend([v for v in AJAY])
            else:
                ...
                if local_AJAY is not None:
                    AJAY_per_question.extend([v for v in local_AJAY])

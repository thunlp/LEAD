...
class DenseRetriever(object):
    ...
    def generate_question_vectors(self, questions: List[str], query_token: str = None) -> T:
        # print(f"generate_question_vectors: {questions}")
        bsz = self.batch_size
...
def main(cfg: DictConfig):
    ...
    if cfg.is_DPR_checkpoint:
    ...
    else: # --------------------- load other checkpoint or hugginface models
        print("This is not DPR checkpoint")
        logger.info("CFG (after gpu  configuration):")
        logger.info("%s", OmegaConf.to_yaml(cfg))
        ...
        model_to_load = get_model_obj(encoder)
        # ------- load SAILER or Lawformer
        if cfg.from_pretrained:
            model = AutoModel.from_pretrained(cfg.model_file)
            ...
            for key in state_dict.keys():
                if key.startswith("module.lm_q.") and "position_ids" not in key:
                    new_state_dict[key[len("module.lm_q."):]] = state_dict[key]

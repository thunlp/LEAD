...
parser.add_argument('--w', type=str, default='data/prediction/bm25_top100_new.json', help='Write path.')
...
for line in tqdm(lines[:]):
    ...
    # if eval(line)['ridx'] == 5156:
    #     print(bm25Model.get_scores(q)[33568], bm25Model.get_scores(q)[38633])
    #     exit(0)
    rankdic[eval(line)['ridx']] = np.array(bm25Model.get_scores(q)).argsort()[-101:].tolist() # argsort返回了从小到大的索引, 所以取后面的101个, 这样排序的顺序其实分数是从小到大的

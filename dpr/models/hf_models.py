...
def get_bert_tokenizer(pretrained_cfg_name: str, do_lower_case: bool = True):
    tokenizer = BertTokenizer.from_pretrained(pretrained_cfg_name, do_lower_case=do_lower_case)
    return tokenizer
...
class BertTensorizer(Tensorizer):
    ...
    def text_to_tensor(
        ...
        ctx_vectors_generate: bool = False,
    ):
        text = text.strip()
        # tokenizer automatic padding is explicitly disabled since its inconsistent behavior
        # TODO: move max len to methods params?
        if not ctx_vectors_generate:
            if title:
        ...
        else: # NOTE When generating vector representations of documents for evaluation, the 'else' part will run. Here, if the document length exceeds sequence length, it will be automatically split into chunks, and the final similarity score of the document will be the highest score among all chunks
            apply_max_len = False 
            if title:
                token_ids = self.tokenizer.encode(
                    title,
                    text_pair=text,
                    add_special_tokens=add_special_tokens,
                    max_length=self.max_length if apply_max_len else 100000,
                    pad_to_max_length=False,
                    truncation=True,
                )
            else:
            ...
            if len(token_ids) >= seq_len:
                ...
                def split_list_into_segments(input_list, seq_len):
                    segments = []
                    for i in range(0, len(input_list), seq_len - 2):
                        segment = [self.tokenizer.cls_token_id] + input_list[i:i + seq_len - 2] + [self.tokenizer.sep_token_id]
                        assert len(segment) <= seq_len
                        assert segment[0] == self.tokenizer.cls_token_id
                        assert segment[-1] == self.tokenizer.sep_token_id
                        segments.append(segment)
                    return segments
                token_ids_list = split_list_into_segments(token_ids, seq_len)
                if self.pad_to_max and len(token_ids_list[-1]) < seq_len:
                    token_ids_list[-1] = token_ids_list[-1] + [self.tokenizer.pad_token_id] * (seq_len - len(token_ids_list[-1]))
                return [torch.tensor(token_ids) for token_ids in token_ids_list]
            return [torch.tensor(token_ids)]

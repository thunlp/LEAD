...
class BiEncoderNllLoss(object):
    def calc(
        ...
        """
        # For each question, find its corresponding AJAY. If the AJAYs of question i and question j are the same, set the score of question i for ctx j to negative infinity, and set the score of question j for ctx i to negative infinity.
        scores = self.get_scores(q_vectors, ctx_vectors)
        # NOTE uncomment this if false negatives masking strategy is not needed
        # AJAY_list = None 
        if AJAY_list is not None:
            for i in range(len(AJAY_list)):
                for j in range(i+1, len(AJAY_list)):
                    if AJAY_list[i] == AJAY_list[j]:

...
class HFBertEncoder(BertModel):
    def __init__(self, config, project_dim: int = 0):
        BertModel.__init__(self, config, add_pooling_layer=False) # TODO
    ...
    def init_encoder(
        ...
        cfg = BertConfig.from_pretrained(cfg_name if cfg_name else "bert-base-uncased") ############################# TODOgao pool
        if dropout != 0:
...
class BertTensorizer(Tensorizer):
    ...
    def text_to_tensor(
        ...
        if not ctx_vectors_generate:
            ...
            apply_max_len = False # TODOgao
            if title:
                token_ids = self.tokenizer.encode(
                    title,
                    text_pair=text,
                    add_special_tokens=add_special_tokens,
                    max_length=self.max_length if apply_max_len else 100000, # TODOgao
                    pad_to_max_length=False,
                    truncation=True,
                )
            else:
            ...
            if self.pad_to_max and len(token_ids) < seq_len:
                token_ids = token_ids + [self.tokenizer.pad_token_id] * (seq_len - len(token_ids))
            if len(token_ids) >= seq_len: # TODOgao
                token_ids = token_ids[0:seq_len] if apply_max_len else token_ids
                token_ids[-1] = self.tokenizer.sep_token_id

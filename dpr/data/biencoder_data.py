...
class T2RankingDataset(Dataset):
    ...
    def _load_all_data(self):
        ...
        # print("this is the unique index", self.query.index.unique())
        self.query.pop('qid')
        ...
        if len(top1000.columns)==3:
            ...
            print("wrong neg file format, length of columns is ", len(top1000.columns))
            exit(0)
...
class LCRDataset(Dataset):
    ...
    def _load_all_data(self):
        with open(self.file, 'r') as f:
            data = json.load(f)
        # filter those without positive ctx
        self.data = [{"question": r["query"],
                      "positive_ctxs": [{"title": r["title"], "text": r["key"]}],
                      "hard_negative_ctxs": [] if "hard_negative_ctxs" not in r else r["hard_negative_ctxs"],
                      "AJAY": r["AJAY"] if "AJAY" in r else None} 
                      for r in data]
        logger.info("Total cleaned data size: %d", len(self.data))
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        ...
        r.negative_passages = [create_passage(ctx) for ctx in negative_ctxs]
        r.hard_negative_passages = [create_passage(ctx) for ctx in hard_negative_ctxs]
        if "AJAY" in json_sample and json_sample["AJAY"]:
            AJAY_list = json_sample["AJAY"]
            AJAY_list.sort()
            r.AJAY = AJAY_list
        else:
            r.AJAY = None

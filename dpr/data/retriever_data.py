...
class JsonLawQASrc(QASrc):
    ...
    def load_data(self):
        if self.ridx == -1:
            print("Test all queries in this file")
        super().load_data()
        qa = []
        with open(self.file, "r") as f:
            data = json.load(f)
        for _dict in data:
            if self.ridx == -1 or ("ridx" in _dict and _dict["ridx"] == self.ridx):
...
class CAIL2019SCMQASrc(QASrc):
    ...
    def load_data(self):
        ...
        for i, d in enumerate(dicts):
            question = d[self.question_attr]
            answers = d[self.answers_attr] if self.answers_attr in d else []
            id = str(i)
            data.append(QASample(self._process_question(question), id, answers))
        self.data = data
        ...
        for file in os.listdir(self.file):
            with open(os.path.join(self.file, file)) as f:
                ...
                if self.id_prefix:
                    ...
                    sample_id = file[:-5]
                passage = data[self.fact_key]
                if self.normalize:
                    passage = normalize_passage(passage)
                ctxs[sample_id] = BiEncoderPassage(passage, str(data[self.title_key]))

class CAIL2019SCMCtxSrc(RetrieverData):
    def __init__(
        self,
        id_prefix: str = None,
        file: str = None,
    ...
    def load_data_to(self, ctxs: Dict[object, BiEncoderPassage]):
        super().load_data()
        logger.info("Reading CAIL2019-SCM file %s", self.file)
        with open(self.file, 'r') as f:
            dicts = json.load(f)
        for i, data in enumerate(dicts):
            if self.id_prefix:
                sample_idC = self.id_prefix + str(i) + "C"
                sample_idB = self.id_prefix + str(i) + "B"
            else:
                sample_idC = str(i) + "C"
                sample_idB = str(i) + "B"
            passageC = data["C"]
            passageB = data["B"]
            if self.normalize:
                passageC = normalize_passage(passageC)
                passageB = normalize_passage(passageB)
            ctxs[sample_idC] = BiEncoderPassage(passageC, sample_idC)
            ctxs[sample_idB] = BiEncoderPassage(passageB, sample_idB)
